-- Fix Google/social signup: profiles.tradition and profiles.circadian_goal are NOT NULL
-- without defaults on production — handle_new_user INSERT was failing.

ALTER TABLE public.profiles
  ALTER COLUMN tradition SET DEFAULT 'general',
  ALTER COLUMN circadian_goal SET DEFAULT 'better_dreams';

UPDATE public.profiles
SET
  tradition = COALESCE(NULLIF(tradition, ''), 'general'),
  circadian_goal = COALESCE(NULLIF(circadian_goal, ''), 'better_dreams')
WHERE tradition IS NULL OR tradition = '' OR circadian_goal IS NULL OR circadian_goal = '';
