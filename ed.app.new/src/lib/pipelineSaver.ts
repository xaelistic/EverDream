...
export async function autoSavePipelineResults(
  ...
  try {
    ...
    const dreamRecord: Partial<DreamRecord> = {
      ...
      nugget: analysis?.nugget ?? undefined,
      interpretation: (analysis?.interpretation as unknown as Record<string, unknown>) ?? undefined,
      mood_valence: calculateValence(analysis) ?? undefined,
      generated_image_url: image?.url ?? undefined,
      generated_image_prompt: image?.prompt ?? undefined,
      generated_image_style: image?.style || 'dreamlike',
      generated_image_source: image?.source ?? undefined,
      visibility,
      is_sample: false,
      is_deleted: false,
    };
